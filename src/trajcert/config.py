...
type YamlValue = None | bool | int | float | str | tuple['YamlValue', ...] | Mapping[str, 'YamlValue']
...
class FailureBoundaryConfig(ConfigModel):
    ...
    def validate_failure_boundary_axes(self) -> FailureBoundaryConfig:
        for field_name, values in (('failure_boundary.unresolvedness', self.unresolvedness), ('failure_boundary.timing_contrast', self.timing_contrast), ('failure_boundary.prevalence', self.prevalence), ('failure_boundary.bands', self.bands), ('failure_boundary.information_margin', self.information_margin), ('failure_boundary.risk_offset', self.risk_offset), ('failure_boundary.sample_size', self.sample_size)):
...
class TrajCertConfig(ConfigModel):
    ...
    @staticmethod
    def freeze_law_mapping(value: Mapping[LawKey, LawConfig]) -> Mapping[LawKey, LawConfig]:
    ...
    def validate_cross_field_contracts(self) -> TrajCertConfig:
        ...
        if any(bool(x) for x in (finest_bands % band_count != 0 for band_count in self.grids.partitions)):
            raise ValueError('every configured analysis partition must deterministically coarsen the finest partition')
        if any(bool(x) for x in (band_count > finest_bands for band_count in self.grids.partitions)):
    ...
    def with_runner_overrides(self, overrides: RunnerOverrides) -> TrajCertConfig:
        ...
        if overrides.sequential is not None:
            if overrides.sequential.coverage is not None:
                coverage = SequentialCoverageConfig.model_validate(coverage.model_dump() | overrides.sequential.coverage.model_dump(exclude_none=True))
            if overrides.sequential.utility is not None:
                utility = SequentialUtilityConfig.model_validate(utility.model_dump() | overrides.sequential.utility.model_dump(exclude_none=True))
        if overrides.benchmark is not None:
            benchmark = BenchmarkConfig.model_validate(benchmark.model_dump() | overrides.benchmark.model_dump(exclude_none=True))
        return self.model_copy(update={'sequential': self.sequential.model_copy(update={'coverage': coverage, 'utility': utility}), 'benchmark': benchmark})
...
def load_config_with_runner_overrides(production_path: Path, override_path: Path) -> TrajCertConfig:
    ...
    try:
        overrides = RunnerOverrides() if payload is None else RunnerOverrides.model_validate(payload)
...
def _require_unique[HashableValue: Hashable](values: tuple[HashableValue, ...], field_name: str) -> None:
    if len(values) != len(set(values)):
        raise ValueError(f'{field_name} contains duplicate values')

def _require_strictly_increasing(values: tuple[int, ...] | tuple[float, ...], field_name: str) -> None:
...
def _require_strictly_decreasing(values: tuple[int, ...] | tuple[float, ...], field_name: str) -> None:
    if any(bool(x) for x in ((left <= right for left, right in pairwise(values)))):
