...
def _stable_resistance_objective(data: _CallbackData, hidden: mpf) -> mpf:
    callback_config = active_config.get().comparators.callback
    first = _log_odds_ratio(data, callback_config.stable_resistance_first_band, hidden)
    second = _log_odds_ratio(data, callback_config.stable_resistance_second_band, hidden)
