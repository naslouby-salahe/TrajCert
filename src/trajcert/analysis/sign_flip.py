...
def one_sided_sign_flip(
    ...
    favorable_or_more_extreme = 0  # TODO: should be constant
    for _ in range(randomization_count):
        bits: NDArray[np.int8] = rng.integers(0, 2, size=values.size, dtype=np.int8)  # TODO: should be constant
        multipliers: NDArray[np.float64] = bits.astype(np.float64)
        multipliers *= 2.0  # TODO: should be constant
        multipliers -= 1.0  # TODO: should be constant
    ...
    p_value = (1.0 + favorable_or_more_extreme) / (1.0 + randomization_count)  # TODO: should be constant
