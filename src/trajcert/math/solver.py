...
def solve_hidden_mass_interval(summary: ObservableSummary, sensitivity_budget: SensitivityBudget, root_atol: ToleranceValue, identity_atol: ToleranceValue) -> HiddenMassSolveResult:
    ...
    if compatibility.regime is CompatibilityRegime.MODEL_INCOMPATIBLE:
        return HiddenMassSolveResult(compatibility=compatibility, interval=None, lower_root=None, upper_root=None)
    if compatibility.regime is CompatibilityRegime.NO_RESOLVED_MASS:
        return HiddenMassSolveResult(compatibility=compatibility, interval=HiddenMassInterval(lower=0.0, upper=unresolved), lower_root=None, upper_root=None)
    minimum = compatibility.minimum_information_point
    if minimum is None:
        raise InvariantViolationError('compatible nondegenerate case is missing its information minimum')
    ...
    if compatibility.regime is CompatibilityRegime.NO_UNRESOLVED_MASS:
        ...
        return HiddenMassSolveResult(compatibility=compatibility, interval=HiddenMassInterval(lower=0.0, upper=0.0), lower_root=lower, upper_root=upper)
    if compatibility.regime is CompatibilityRegime.MINIMUM_INFORMATION_SINGLETON:
        ...
        return HiddenMassSolveResult(compatibility=compatibility, interval=HiddenMassInterval(lower=u_dagger, upper=u_dagger), lower_root=lower, upper_root=upper)
    lower_root = _solve_lower_branch(summary=summary, rho=rho, u_dagger=u_dagger, root_atol=root_tolerance, identity_atol=identity_tolerance)
    upper_root = _solve_upper_branch(summary=summary, rho=rho, u_dagger=u_dagger, unresolved=unresolved, root_atol=root_tolerance, identity_atol=identity_tolerance)
    return HiddenMassSolveResult(compatibility=compatibility, interval=HiddenMassInterval(lower=lower_root.root, upper=upper_root.root), lower_root=lower_root, upper_root=upper_root)

def _solve_lower_branch(summary: ObservableSummary, rho: float, u_dagger: float, root_atol: float, identity_atol: float) -> RootBracket:
    ...
    return _bisect(summary=summary, rho=rho, branch=RootBranch.LOWER, lower=0.0, upper=u_dagger, lower_residual=boundary_value, upper_residual=minimum_value, root_atol=root_atol, identity_atol=identity_atol)

def _solve_upper_branch(summary: ObservableSummary, rho: float, u_dagger: float, unresolved: float, root_atol: float, identity_atol: float) -> RootBracket:
    ...
    return _bisect(summary=summary, rho=rho, branch=RootBranch.UPPER, lower=u_dagger, upper=unresolved, lower_residual=minimum_value, upper_residual=boundary_value, root_atol=root_atol, identity_atol=identity_atol)

def _bisect(*, summary: ObservableSummary, rho: float, branch: RootBranch, lower: float, upper: float, lower_residual: float, upper_residual: float, root_atol: float, identity_atol: float) -> RootBracket:
    ...
    result = RootBracket(branch=branch, status=RootStatus.BISECTION, lower=lower, upper=upper, width=upper - lower, root=root, residual=residual, iterations=iterations)
...
def _exact_root(branch: RootBranch, hidden_mass: float, summary: ObservableSummary, rho: float, status: RootStatus) -> RootBracket:
    residual = abs(_profile_residual(summary, hidden_mass, rho))
    return RootBracket(branch=branch, status=status, lower=hidden_mass, upper=hidden_mass, width=0.0, root=hidden_mass, residual=residual, iterations=0)

def _require_residual(root: RootBracket, identity_atol: float) -> None:
