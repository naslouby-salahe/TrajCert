...
def observed_timing_information(summary: ObservableSummary) -> InformationNats | None:
    ...
    if resolved_mass == 0.0:
        return None
    marginal_entropy = binary_entropy_from_masses(summary.resolved_harmful_mass, summary.resolved_correct_mass)
...
def minimum_information_point(summary: ObservableSummary) -> MinimumInformationPoint | None:
    ...
    return MinimumInformationPoint(hidden_terminal_harmful_mass=hidden_mass, latent_risk=latent_risk, information_floor=information_floor)
...
def information_profile(summary: ObservableSummary, hidden_terminal_harmful_mass: Mass) -> InformationNats:
    hidden = _hidden_mass(summary, hidden_terminal_harmful_mass)
    harmful = summary.resolved_harmful_mass
    unresolved = summary.unresolved_mass
    timing_entropy = resolved_timing_entropy(summary)
...
def information_profile_derivative(summary: ObservableSummary, hidden_terminal_harmful_mass: Mass) -> InformationDerivative:
...
def information_profile_second_derivative(summary: ObservableSummary, hidden_terminal_harmful_mass: Mass) -> InformationCurvature:
    hidden = _strictly_interior_hidden_mass(summary, hidden_terminal_harmful_mass)
    harmful = summary.resolved_harmful_mass
    correct = summary.resolved_correct_mass
    unresolved = summary.unresolved_mass
...
def timing_gain(fine: ObservableSummary, coarse: ObservableSummary, identity_tolerance: ToleranceValue) -> InformationNats:
    tolerance = identity_tolerance
    if not isfinite(tolerance) or tolerance <= 0.0:
        raise InvalidScientificDataError('identity tolerance must be finite and positive')
    for fine_value, coarse_value, field_name in ((fine.resolved_harmful_mass, coarse.resolved_harmful_mass, 'resolved harmful mass'), (fine.resolved_correct_mass, coarse.resolved_correct_mass, 'resolved correct mass'), (fine.unresolved_mass, coarse.unresolved_mass, 'unresolved mass')):
...
def profile_difference(fine: ObservableSummary, coarse: ObservableSummary, hidden_terminal_harmful_mass: Mass, identity_tolerance: ToleranceValue) -> InformationNats:
    timing_gain(fine, coarse, identity_tolerance)
    return information_profile(fine, hidden_terminal_harmful_mass) - information_profile(coarse, hidden_terminal_harmful_mass)
