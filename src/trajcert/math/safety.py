...
def assess_safety_geometry(
    ...
    minimum = minimum_information_point(
        summary
    )
...
def safety_budget_cases(
    ...
    if minimum is None:
        return (
            SafetyBudgetCase(
                ...
                risk_budget=RiskBudget(
                    max(
                        0.0,
                        harmful
                        - _RESOLVED_HARM_BOUNDARY_OFFSET,
                    )
                ),
                valid=True,
                invalid_reason=None,
            ...
            SafetyBudgetCase(
                name=SafetyCaseName(
                    "Between resolved mass "
                    "and intrinsic boundary"
                ),
                risk_budget=None,
                valid=False,
                invalid_reason=ReasonCode(
                    "DEGENERATE_SAFETY_INTERVAL"
                ),
            ),
            SafetyBudgetCase(
                name=SafetyCaseName(
