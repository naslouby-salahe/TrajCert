...
def assess_safety_geometry(
    ...
    if minimum is None:
        return SafetyAssessment(
            ...
            risk_budget=RiskBudget(beta),
            resolved_harmful_mass=RiskValue(harmful),
            minimum_information_risk=None,
            assumption_free_upper=RiskValue(assumption_free_upper),
...
def safety_budget_cases(
        ...
        return (
            SafetyBudgetCase(
                ...
                risk_budget=RiskBudget(
                    max(
                        0.0,
                        harmful - _RESOLVED_HARM_BOUNDARY_OFFSET,
                    )
                ),
                valid=True,
                invalid_reason=None,
            ...
            SafetyBudgetCase(
                ...
                risk_budget=RiskBudget(theta_max),
                valid=True,
                invalid_reason=None,
            ),
        )
    ...
    return (
        ...
        SafetyBudgetCase(
            name=SafetyCaseName("Between resolved mass and intrinsic boundary"),
            risk_budget=(RiskBudget((harmful + theta_dagger) / 2.0) if between_is_valid else None),
            valid=between_is_valid,
            invalid_reason=(None if between_is_valid else ReasonCode("DEGENERATE_SAFETY_INTERVAL")),
        ),
        SafetyBudgetCase(
            name=SafetyCaseName("At intrinsic boundary"),
            risk_budget=RiskBudget(theta_dagger),
            valid=True,
            invalid_reason=None,
        ),
        SafetyBudgetCase(
            name=SafetyCaseName("Interior safety frontier"),
            risk_budget=RiskBudget((theta_dagger + theta_max) / 2.0),
            valid=True,
            invalid_reason=None,
        ),
        SafetyBudgetCase(
