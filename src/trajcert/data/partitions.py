...
class TrajectoryPartition(DomainModel):
    ...
    def __post_init__(self) -> None:
        ...
        if bands > finest or finest % bands != 0:
            raise InvalidPartitionError('partition must be a deterministic coarsening of the finest partition')
        ...
        if mapped != _coarsening_map_values(finest, bands):
            raise InvalidPartitionError('coarsening map is inconsistent with deterministic equal-width coarsening')
...
def build_partition(finest_band_count: BandCount, band_count: BandCount, terminal_horizon: TerminalHorizon) -> TrajectoryPartition:
...
def _is_refinement(fine: TrajectoryPartition, coarse: TrajectoryPartition) -> bool:
    return fine.finest_band_count == coarse.finest_band_count and fine.terminal_horizon == coarse.terminal_horizon and (fine.band_count >= coarse.band_count) and (fine.band_count % coarse.band_count == 0)

def coarsen_mass_vector(values: Vector, fine: TrajectoryPartition, coarse: TrajectoryPartition) -> Vector:
