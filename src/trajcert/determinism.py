...
def derive_seed(namespace: SeedNamespace, index: SeedIndex) -> SeedValue:
    ...
    material = SEED_FIELD_SEPARATOR.join((SEED_PREFIX, str(namespace), str(numeric_index))).encode('utf-8')
...
def event_stream_namespace(law_name: LawName, band_count: BandCount) -> SeedNamespace:
    ...
    return SeedNamespace(f'{SeedNamespaceRole.EVENT_STREAM.value}{SEED_FIELD_SEPARATOR}law={law_name}{SEED_FIELD_SEPARATOR}K={int(band_count)}')
...
def _descriptor_namespace(role: SeedNamespaceRole, descriptor: SemanticComparisonKey) -> SeedNamespace:
    if not descriptor:
        raise InvalidScientificDataError('seed namespace descriptor cannot be empty')
    if descriptor != descriptor.strip():
        raise InvalidScientificDataError('seed namespace descriptor cannot contain leading or trailing whitespace')
