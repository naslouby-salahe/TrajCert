...
def event_stream_namespace(
    ...
    return SeedNamespace(
        ...
        f"{SEED_FIELD_SEPARATOR}law={law_name}"
        f"{SEED_FIELD_SEPARATOR}K={int(band_count)}"
    )
...
def bootstrap_namespace(
    ...
    return _descriptor_namespace(
        SeedNamespaceRole.BOOTSTRAP,
        semantic_comparison_key,
    )


def permutation_namespace(
    semantic_comparison_key: SemanticComparisonKey,
) -> SeedNamespace:
    return _descriptor_namespace(
