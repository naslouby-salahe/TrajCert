...
def _baseline_rows(config: TrajCertConfig) -> tuple[BaselineAssumptionRow, ...]:
    ...
    return (
        _baseline(
            "Complete-case arrival-only",
            "descriptive complete-case reference",
            "resolved outcomes only",
            "terminal unresolved mass ignored",
            "A/(A+G) when A+G>0",
            "not sensitivity-indexed",
            "paired by law/cell",
            "latent risk",
            "descriptive reference",
            "not a PIS certificate",
        ),
        _baseline(
            "Unresolved-as-harm worst case",
            "assumption-free upper reference",
            "resolved plus terminal mass",
            "all unresolved mass harmful",
            "A+c",
            "not sensitivity-indexed",
            "paired by law/cell",
            "risk upper",
            "assumption-free bound",
            "not evidence of path-information gain",
        ),
        _baseline(
            "Endpoint-only path information",
            "trajectory-resolution ablation",
            "endpoint-only coarsening",
            "same numerical rho as TrajCert",
            "K=1 deterministic coarsening",
            grid,
            "paired by law/rho/stream",
            "risk upper and utility",
            "valid comparator",
            "not universally equivalent to trajectory information",
        ),
        _baseline(
            "Legacy bandwise odds-ratio sensitivity",
            "legacy sensitivity comparator",
            "finite-band response hazards",
            "common Gamma bound across informative bands",
            "analytic linear-rational feasibility",
            "configured legacy Gamma grid",
            "deterministic",
            "feasible risk set",
            "legacy comparator",
            "no universal Gamma-to-rho mapping",
        ),
        _baseline(
            "ALHO common-slope callback",
            "callback-model reduction comparator",
            "informative finite bands",
            "common log-odds slope",
            "10001-point high-precision root search with fixed acceptance tolerance",
            "callback-specific",
            "deterministic",
            "risk set",
            "configured callback model",
            "not a generic information model",
        ),
        _baseline(
            "Stable-resistance callback",
            "callback-model reduction comparator",
            "first two informative bands",
            "equal response-hazard log odds",
            "high-precision deterministic root search",
            "callback-specific",
            "deterministic",
            "risk set",
            "K>=2 only",
            "not applicable when fewer than two informative bands remain",
        ),
        _baseline(
            "Binary repeated-attempt pattern mixture",
            "parametric repeated-attempt comparator",
            "finite resolved bands",
            "logit-linear response pattern",
            "bounded L-BFGS-B with fixed stability checks",
            "configured pattern-mixture C grid",
            "deterministic",
            "latent risk",
            "numerically stable fitted cells",
            "not a nonparametric guarantee",
        ),
        _baseline(
            "Generic full-law information oracle",
            "independent numerical validation",
            "full 2x(K+1) law",
            "only information-budget model",
            "independent high-precision direct-table optimization",
            grid,
            "deterministic",
            "oracle endpoint error",
            "validation only",
            "not production implementation",
        ),
        _baseline(
            "Time-uniform observable-law projection",
            "sequential valid-bound reference",
            "all matured observable categories",
            "time-uniform categorical CS",
            "certified outer projection",
            _display_value(config.sequential.utility.rho),
            "same streams as TrajCert",
            "anytime upper risk",
            "valid sequential reference",
            "does not include TrajCert state semantics",
        ),
        _baseline(
            "Repeated-static monitoring negative control",
            "continuous-monitoring negative control",
            "current matured category counts",
            "no across-time correction",
            "per-time Wilson/Bonferroni projection",
            _display_value(config.sequential.utility.rho),
            "same streams as TrajCert",
            "ever violation and utility",
            "negative control only",
            "cannot support deployment",
        ),
        _baseline(
            "Ignorable-delay anytime reference",
            "assumption-dependent sequential reference",
            "resolved labels among matured events",
            "L independent of J",
            "Jeffreys beta-binomial mixture confidence sequence",
            _display_value(config.sequential.utility.rho),
            "same streams when assumption valid",
            "anytime upper risk",
            "only cells satisfying ignorable delay",
            "excluded when assumption violated",
        ),
    )
