...
def test_partition_and_observable_law_contract() -> None:
    ...
    assert partition.band_for_age(0) == 1
    assert partition.band_for_age(3) == 2
    assert partition.band_for_age(8) == 3
    assert partition.band_for_age(9) is None
    assert math.isclose(0.2, law.harmful_total)
    assert math.isclose(0.5, law.correct_total)
    assert math.isclose(law.latent_risk(0.2), 0.4)
